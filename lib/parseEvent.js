...
async function parseEvent(auth, agendaJson) {
  try {
    const dateStart = moment(agendaJson[0].date, "DD/MM/YYYY").format();
    const dateStartiso = moment(dateStart).toISOString();
    console.log(dateStartiso);
    let delay = 2000; // Délai initial en millisecondes
    await deleteEvents(auth, delay, dateStartiso);
...
async function deleteEvents(auth, delay, date) {
  const calendar = google.calendar({ version: "v3", auth });
